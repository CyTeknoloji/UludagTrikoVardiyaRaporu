...
class RaporEkleViewModel:ViewModel() {
    ...
    fun raporKaydet(rapor: Rapor,context: Context,activity: Activity){
        when{
            ...
            else->kaydet(rapor,context, activity)
        }
    }

    private fun kaydet(rapor: Rapor,context: Context,activity: Activity){
        uploadLiveData.value=true
    ...
    fun getDateFromPicker(context:Context){
        ...
        val month=calendar.get(Calendar.MONTH)
        val day=calendar.get(Calendar.DAY_OF_MONTH)
        val datePicker=DatePickerDialog(context,DatePickerDialog.OnDateSetListener { view, y, m, dayOfMonth ->
            val newM=m+1
