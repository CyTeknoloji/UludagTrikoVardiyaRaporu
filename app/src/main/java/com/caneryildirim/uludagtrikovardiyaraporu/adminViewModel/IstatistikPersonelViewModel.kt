...
class IstatistikPersonelViewModel:ViewModel() {
    private val personelList=ArrayList<String>()
    ...
    fun getIstatistikData(view: View, context: Context){
        ...
        firestore.collection("istatistikpersonel").whereEqualTo("email",auth.currentUser!!.email)
    ...
    fun deleteIstatistik(docRef:String, context: Context, view: View){
        uploadLiveData.value=true
        firestore.collection("istatistikpersonel").document(docRef).delete().addOnSuccessListener {
            uploadLiveData.value=false
    ...
    fun filterPersonel(viewReq: View, layoutInflater: LayoutInflater, context: Context, activity: AdminActivity){
        var personel=""
        ...
        builder.show()
        bindingAlert.textView20.text="Personel"


        val personelFilterListSet=personelList.toHashSet()
        val personelFilterList=ArrayList<String>()
        personelFilterList.clear()
        personelFilterList.add("Personel Seçiniz")
        personelFilterListSet.forEach {
            personelFilterList.add(it)
        }

        val vardiyaFilterAdapter= ArrayAdapter(context, R.layout.spinner_item, R.id.spinner_text,personelFilterList)
        ...
        bindingAlert.spinnerVardiyaFilter.onItemSelectedListener=object : AdapterView.OnItemSelectedListener{
            override fun onItemSelected(p0: AdapterView<*>?, p1: View?, p2: Int, p3: Long) {
                personel=personelFilterList[p2]
            }

            override fun onNothingSelected(p0: AdapterView<*>?) {
                personel=personelFilterList[0]
            }
        ...
        bindingAlert.buttonUygulaFilter.setOnClickListener {
            when{
                ...
                personel=="Personel Seçiniz"-> Toast.makeText(context,"Personel seçiniz", Toast.LENGTH_SHORT).show()
                else->{
                    ...
                    getRaporData(viewReq,startDate,endDate,personel,context,tarihAralik)
                    builder.cancel()
                }
            }
        }
    ...
    fun getRaporData(view: View,startDate: Timestamp, endDate: Timestamp, personel:String,context: Context,tarihAralik:String){
        uploadLiveData.value=true
        firestore.collection("raporlar").whereEqualTo("personel",personel)
                ...
                if (it!=null){
                    if (!it.isEmpty){
                        ...
                        for (document in documents){
                            ...
                            val makina=document.get("makina") as String
                            val vardiya=document.get("vardiya") as String
                            val personelRapor=document.get("personel") as String
                            ...
                            val istatistik=Istatistik(tarih, makina, vardiya, personelRapor, calismaSuresi, uretimYuzdesi,
                                uretimAdedi, sure, fireAdedi, kirilanIgne, kirilanPlatin)
                            istatistikList.add(istatistik)
                        }
                        resultFilter(view,context,startDate, endDate, personel, istatistikList,tarihAralik)
                    }else{
                        //emty
                        uploadLiveData.value=false
                        Toast.makeText(context,"Seçtiğiniz Personele ait yüklenmiş rapor yok",Toast.LENGTH_LONG).show()
                        println("emty")
                    }
                }else{
                    //null
    ...
    private fun resultFilter(view: View,context: Context,startDate: Timestamp, endDate: Timestamp, personel:String,istatistikList:ArrayList<Istatistik>,tarihAralik: String){
        ...
        firestore.collection("istatistikpersonel").document(docRef).set(istatistikRaporHashMap)
        ...
        if (istatistikListForExcel.size>0){
            ...
            refStorage.putFile(uri).addOnSuccessListener {
                refStorage.downloadUrl.addOnSuccessListener {
                    downloadFile(it.toString(),"istatistikpersonel-${day}-${month+1}-${year}.xls",context)
                }.addOnFailureListener {
                    uploadLiveData.value=false
                    Toast.makeText(context,"Hata oluştu.Tekrar deneyiniz",Toast.LENGTH_SHORT).show()
                }
    ...
    private fun getPersonelData(){
        firestore.collection("raporlar").orderBy("personel",Query.Direction.ASCENDING).get()
            .addOnSuccessListener {
                if (it!=null){
                    if (!it.isEmpty){
                        personelList.clear()
